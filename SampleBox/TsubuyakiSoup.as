...
#cfunc _CryptEncrypt "CryptEncrypt" int, int, int, int, int, int, int
#cfunc _CryptDecrypt "CryptDecrypt" int, int, int, int, int, int
//---------------
//  wininet.dll
//---------------
#uselib "wininet.dll"
	...
	if ( _CryptCreateHash(hProv, 0x00008004, 0, 0, hHash) ) {
		...
		if ( _CryptHashData(hHash, p2, strlen(p2)+1, 0) ) {
			...
			if ( _CryptDeriveKey(hProv, 0x00006801, hHash, 0x800000, hKey) ) {
				//暗号化
				if ( _CryptEncrypt( hKey, 0, 1, 0, 0, varptr(EncryptStrLen), 0) ) {		;バッファの確保用
					memexpand p1, EncryptStrLen
					mes EncryptStrLen
					if ( _CryptEncrypt( hKey, 0, 1, 0, varptr(p1), varptr(EncryptStrLen2), varptr(EncryptStrLen)) ) {	;暗号化
						refstat = 1
						mes 
					}
				}
...
#defcfunc Decryption var p1, str p2, str p3
	...
	if ( _CryptCreateHash(hProv, 0x00008004, 0, 0, hHash) ) {
		...
		if ( _CryptHashData(hHash, p2, strlen(p2), 0) ) {
			...
			if ( _CryptDeriveKey(hProv, 0x00006801, hHash, 0x800000, hKey) ) {
				//復号
				if ( _CryptDecrypt( hKey, 0, 1, 0, varptr(p1), varptr(EncryptStrLen)) ) {
					refstat = 1
				}
				_CryptDestroyKey hKey
			}
